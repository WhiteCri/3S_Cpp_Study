...
void LinkedList::sort(bool(* compare)(const Node * data1, const Node * data2))
{
	Node  *target;
	HighSchoolNode temp;
	Node * i, *j;

	for(i = head->next;i != this->tail->prev; i = i -> next){
		target = i;
		for(j = i->next; j != this->tail ; j = j ->next){
			if(compare(target,j) == true){
				target = j;
			}
		}
		temp = target->info;
